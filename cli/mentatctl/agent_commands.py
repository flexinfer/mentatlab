...
agent = typer.Typer(
    name="agent", help="Agent lifecycle management commands", no_args_is_help=True
)
...
def create_agent(
    ...
    directory: Optional[Path] = typer.Option(
        None, "--dir", help="Directory to create agent in (default: current directory)"
    ),
    id_prefix: Optional[str] = typer.Option(
        None, "--id", help="Custom agent ID prefix (default: uses name)"
    ),
):
    ...
    try:
        ...
        if not template_dir.exists():
            typer.echo(
                f"❌ Error: Template '{template}' not found at {template_dir}", err=True
            )
            raise typer.Exit(code=1)
        ...
        for template_file in template_dir.rglob("*"):
            if template_file.is_file():
                ...
                content = content.replace(
                    "{{DESCRIPTION}}", f"A {template} agent that processes inputs"
                )
...
def validate_agent(
    ...
    mode: Optional[str] = typer.Option(
        None, "--mode", help="Validation mode (strict, permissive, disabled)"
    ),
):
    ...
    try:
        ...
        try:
            ...
            response = requests.post(
                f"{orchestrator_url}/agents/validate",
            ...
            if response.status_code == 200:
                ...
                typer.echo(
                    f"🔧 Validation mode: {result.get('validation_mode', 'unknown')}"
                )
...
def list_agents(
    search: Optional[str] = typer.Option(
        None, "--search", help="Search term to filter agents"
    ),
    orchestrator_url: str = typer.Option(
        "http://localhost:8001", "--orchestrator-url", help="Orchestrator service URL"
    ),
    registry: Optional[str] = typer.Option(
        None, "--registry", help="Container registry to search"
    ),
):
    ...
    try:
            ...
            if agent_dir.exists():
                for manifest_path in agent_dir.rglob("manifest.yaml"):
                    try:
                        ...
                        if (
                            not search
                            or search.lower() in agent_id.lower()
        ...
        sample_agents = [
            ...
            {
                "id": "flexinfer.openai-chat",
                "version": "1.2.0",
                "description": "OpenAI GPT chat integration",
            },
            {
                "id": "flexinfer.image-classifier",
                "version": "0.5.1",
                "description": "Image classification using ResNet",
            },
            {
                "id": "flexinfer.text-summarizer",
                "version": "1.0.0",
        ...
        for agent in sample_agents:
            if (
                not search
                or search.lower() in agent["id"].lower()
                or search.lower() in agent["description"].lower()
            ):
                typer.echo(f"   • {agent['id']} v{agent['version']}")
                typer.echo(f"     {agent['description']}")
...
def register_agent(
    ...
    update: bool = typer.Option(
        False, "--update", help="Update existing registration instead of creating new"
    ),
):
    """Register an agent with the orchestrator registry"""
    try:
        if not manifest_file.exists():
            typer.echo(
                f"❌ Error: Manifest file not found at {manifest_file}", err=True
        ...
        typer.echo(f"📝 Registering agent: {agent_id} v{version}")
        typer.echo(f"🎯 Orchestrator: {orchestrator_url}")

        # Send registration request
        try:
            endpoint = f"{orchestrator_url}/api/v1/agents"
            if update:
                response = requests.put(
                    f"{endpoint}/{agent_id}", json=manifest_data, timeout=30
                )
            else:
                response = requests.post(endpoint, json=manifest_data, timeout=30)

            if response.status_code in [200, 201]:
                result = response.json()
                typer.echo(f"✅ Agent registered successfully!")
                typer.echo(f"🆔 ID: {result.get('id', agent_id)}")
                typer.echo(f"📦 Image: {result.get('image', 'not set')}")
                if result.get("capabilities"):
                    typer.echo(f"🏷️  Capabilities: {', '.join(result['capabilities'])}")
            elif response.status_code == 409:
                typer.echo(
                    f"⚠️  Agent '{agent_id}' already exists. Use --update to update.",
                    err=True,
                )
                raise typer.Exit(code=1)
            elif response.status_code == 404:
                typer.echo(f"❌ Agent '{agent_id}' not found for update.", err=True)
                raise typer.Exit(code=1)
            else:
                typer.echo(f"❌ Registration failed: {response.text}", err=True)
                raise typer.Exit(code=1)
        ...
        typer.echo(f"❌ Error registering agent: {e}", err=True)
        raise typer.Exit(code=1)


@agent.command("unregister")
def unregister_agent(
    agent_id: str = typer.Argument(..., help="Agent ID to unregister"),
    orchestrator_url: str = typer.Option(
        "http://localhost:7070", "--orchestrator-url", help="Orchestrator service URL"
    ),
    ...
    try:
        if not force:
            confirm = typer.confirm(
                f"Are you sure you want to unregister agent '{agent_id}'?"
            )
            if not confirm:
                typer.echo("Cancelled.")
                raise typer.Exit(code=0)

        typer.echo(f"🗑️  Unregistering agent: {agent_id}")

        try:
            response = requests.delete(
                f"{orchestrator_url}/api/v1/agents/{agent_id}", timeout=30
            )

            if response.status_code == 200:
                typer.echo(f"✅ Agent '{agent_id}' unregistered successfully!")
            elif response.status_code == 404:
                typer.echo(f"⚠️  Agent '{agent_id}' not found.", err=True)
                raise typer.Exit(code=1)
            else:
                typer.echo(f"❌ Unregister failed: {response.text}", err=True)
                raise typer.Exit(code=1)
        ...
        typer.echo(f"❌ Error unregistering agent: {e}", err=True)
        raise typer.Exit(code=1)
...
def push_agent(
    agent_dir: Path = typer.Argument(
        ..., help="Path to agent directory containing manifest.yaml and Dockerfile"
    ),
    registry: str = typer.Option(
        "harbor.lan", "--registry", help="Container registry to push to"
    ),
    tag: Optional[str] = typer.Option(
        None, "--tag", help="Custom tag (default: uses version from manifest)"
    ),
    build_only: bool = typer.Option(
        False, "--build-only", help="Only build, don't push to registry"
    ),
):
    ...
    try:
        ...
        try:
            ...
            typer.echo(
                f"❌ Error: Docker is not available. Please install Docker to build agents.",
                err=True,
            )
            raise typer.Exit(code=1)
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "build", "-t", image_name, str(agent_dir)],
                check=True,
                capture_output=True,
                text=True,
            )
